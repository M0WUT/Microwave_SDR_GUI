...
adau1361::adau1361(const char *iicFile, const int slaveAddress)
{
    ...
    set_datastream_config(
        ...
        FALLING_EDGE,  // BCLK Polarity
        RISING_EDGE,  // LRCLK Polarity
        2,  // Channels Per Frame (2 = stereo, 4 = TDM4)
        SLAVE,  // Is CODEC bus master or slave?
        32,  // Clocks per Frame
        LEFT_FIRST,  // Which ADC channel is sent first
        LEFT_FIRST,  // Which DAC channel is sent first
        MSB_FIRST,  // Data order
        0  // Delay (in BCLK cycles) between start of frame and valid data
    );
